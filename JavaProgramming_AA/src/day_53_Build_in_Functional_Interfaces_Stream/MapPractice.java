...
public class MapPractice {
    public static void main(String[] args) {
        ...
        System.out.println("Java topic and its hours:");
        ...
        System.out.println("----------------------------");
        System.out.println("The names of the java topics that we spend more than 7 hours:");
        for (Map<String, Double> eachUnit : allUnits) {
            for (Map.Entry<String, Double> entry : eachUnit.entrySet()) {
                if(entry.getValue() >= 7){
        ...
        System.out.println("----------------------------");
        System.out.println("The names of the java topics that we spend less than 3 hours:");
        ...
        String javaTopicMaxHour = "";
        String javaTopicMinHour = "";
        for (Map<String, Double> eachUnit : allUnits) {
            for (Map.Entry<String, Double> entry : eachUnit.entrySet()) {
                if (entry.getValue()>maxHour){
                    maxHour = entry.getValue();
                    javaTopicMaxHour = entry.getKey();
                }

                if (entry.getValue()<minHour){
                    minHour = entry.getValue();
                    javaTopicMinHour = entry.getKey();
                }

            }
        }
        System.out.println(javaTopicMaxHour + " has the maximum hours, " + maxHour + " hours");
        System.out.println(javaTopicMinHour + " has the minimum hours, " + minHour + " hours");
